...
def search():
    try:
        with open("password.json","r") as file:
            ...
            password_field.delete(0,END)
            password_field.insert(0,jason_file[web_text_field.get()]["password"])
            password_field.clipboard_clear()
            password_field.clipboard_append(jason_file[web_text_field.get()]["password"])
    except:
        massagebox.showerror("Oops","NO ENTITY FOUNDED")
...
# entry
web_text_field = Entry(window, font=("Arial", 16),width=21)
...
web_text_field.grid(row=1, column=1,padx=(10,0))
add_button = Button(window, text="Add", font=("Arial", 14),width=35,command=add_password)
search_button = Button(window,text="Search", font=("Arial", 14),width=14,command=search)
add_button.grid(row=4,column=1,columnspan=2,pady=(0,30),padx=(0,0))
search_button.grid(row=1,column=2,padx=(0,10))
